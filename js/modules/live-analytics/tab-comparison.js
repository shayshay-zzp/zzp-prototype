...
function renderLiveAnalyticsTabComparison() {
  return card('GMV dự kiến vs thực tế', tableWrap(['Phiên', 'Dự kiến', 'Thực tế', 'Chênh lệch'],
    ZZP_DATA.liveSessions.map(l => {
      ...
      return `<tr class="border-b border-slate-50"><td class="py-3 px-3">${l.title.slice(0, 24)}</td><td class="px-3">${fmt(l.expectedGmv)}</td><td class="px-3">${fmt(l.pastGmv)}</td><td class="px-3 font-semibold">${gap}%</td></tr>`;
    }).join('')));
}
