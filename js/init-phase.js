...
PAGES['products-setup'] = () => {
  ...
  const wizardBanner = !c5?.done ? dsCard('', `
      <div class="ds-flex-between">
        <div style="min-width:0">
          ...
          <p class="ds-page-desc" style="max-width:none;margin-top:8px">Trình 5 bước: thông tin → AI nội dung → AI 6 ảnh → video & tuân thủ → đăng TikTok Shop</p>
  ...
  const listingRows = ZZP_DATA.products.map(p => `<tr ${rowClick('product', p.id)}>
    <td><span style="display:inline-flex;align-items:center;gap:8px">${productThumb(p, 16)}<span style="font-weight:500">${p.name}</span>${p.hero ? ' <span class="ui-badge ui-badge--warn">Chủ lực</span>' : ''}</span></td>
  ...
  return dsPage(dsStack(`
    ...
    ${wizardBanner}
    ${dsCard('Kiểm tra chất lượng tin đăng', dsTable(['Sản phẩm', 'Giá', 'Điểm tin đăng', 'Trạng thái', 'Hành động'], listingRows))}
...
PAGES.store = () => modulePage(
  'Khởi tạo',
  'Store Optimization',
  'Trang trí gian hàng TikTok Shop — xem điểm, ghim SKU, chọn mẫu và xuất bản khi đủ điều kiện',
...
PAGES.compliance = () => {
      ...
      <div class="ds-card" style="margin-bottom:12px;cursor:pointer;border-color:${isAction ? '#fecaca' : 'var(--ds-border)'}" onclick="openDetail('policy','${p.id}')">
        <div class="ds-card-body">
          ...
          <div style="margin-top:12px;padding:12px;background:#fafbfc;border:1px solid var(--ds-border);border-radius:8px" onclick="event.stopPropagation()">
            <p style="margin:0;font-size:12px;font-weight:600;color:var(--ds-brand);display:flex;align-items:center;gap:4px">${icon('sparkles', 12)} Đánh giá AI</p>
  ...
  return dsPage(dsStack(`
    ...
    ${dsCard('Trung tâm chính sách TikTok — Phân tích AI', policies)}
    ${dsCard('Kiểm tra tuân thủ', dsStatGrid([
  ...
  const matrix = `
    <div class="ds-grid ds-grid--2">
      <div class="ds-quadrant ds-quadrant--star">
        <p style="margin:0;font-weight:600;color:var(--ds-brand);display:flex;align-items:center;gap:6px">${icon('star', 14)} Ngôi sao — Mở rộng</p>
      ...
      <div class="ds-quadrant ds-quadrant--cash">
        <p style="margin:0;font-weight:600;color:var(--ds-warning);display:flex;align-items:center;gap:6px">${icon('coins', 14)} Bò sữa</p>
      ...
      <div class="ds-quadrant ds-quadrant--potential">
        <p style="margin:0;font-weight:600;color:var(--ds-info);display:flex;align-items:center;gap:6px">${icon('sprout', 14)} Tiềm năng</p>
      ...
      <div class="ds-quadrant ds-quadrant--review">
        <p style="margin:0;font-weight:600;color:var(--ds-danger);display:flex;align-items:center;gap:6px">${icon('alert-triangle', 14)} Cần xem xét</p>
        <p style="margin:8px 0 0;font-size:13px;display:flex;align-items:center;gap:8px">${productThumb(getProduct('P006'), 14)} Son dưỡng môi — rủi ro tuân thủ</p>
      </div>
    </div>`;
  return dsPage(dsStack(`
    ${pageHeader('Khởi tạo', 'Product Strategy & Hero SKU', 'Xác định sản phẩm chủ lực và chiến lược danh mục')}
    ${dsStatGrid([
      { label: 'SKU chủ lực', value: heroes.length, hint: 'Sản phẩm chiến lược', tone: 'brand' },
      { label: 'Tiềm năng mở rộng', value: '2', hint: 'Serum VC, Kem chống nắng', tone: 'success' },
      { label: 'Cần tối ưu', value: '1', hint: 'Son dưỡng môi P006', tone: 'warning' }
    ])}
    ${dsCard('Ma trận danh mục sản phẩm', matrix)}
  `));
};
