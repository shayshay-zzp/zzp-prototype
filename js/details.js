...
const DETAIL_RENDERERS = {
  ...
  insight: (id) => {
    ...
    if (!i) return notFound('Insight', id);
  ...
  ad: (id) => {
    ...
    if (!ad) return notFound('Ad', id);
    return detailLayout('ad', id, ad.name, 'ads', `
      <div class="grid lg:grid-cols-4 gap-4 mb-6">
        ${statCard('Budget', fmt(ad.budget))}${statCard('Spent', fmt(ad.spent))}${statCard('ROAS', ad.roas ? ad.roas + 'x' : '-', '', ad.roas >= 2 ? 'green' : 'red')}${statCard('Orders', ad.orders)}
      </div>
      ${card('Chi tiết chiến dịch quảng cáo', `
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between"><span class="text-slate-500">Impressions</span><span>${fmt(ad.impressions)}</span></div>
          <div class="flex justify-between"><span class="text-slate-500">Trạng thái</span>${badge(ad.status, ad.status)}</div>
        </div>
        <div class="mt-4 flex gap-2">
          ${ad.roas < 2 && ad.status === 'active' ? `<button onclick="pauseAd('${id}');openDetail('ad','${id}')" class="px-4 py-2 bg-red-600 text-white rounded-lg text-sm">Pause campaign</button>` : ''}
  ...
  sample: (id) => {
    ...
    if (!s) return notFound('Sample', id);
  ...
  content: (id) => {
    ...
    if (!v) return notFound('Content', id);
    const koc = ZZP_DATA.kocs.find(k => k.id === v.koc);
    return detailLayout('content', id, v.title, 'content', `
      <div class="grid lg:grid-cols-4 gap-4 mb-6">
        ${statCard('Views', fmt(v.views))}${statCard('Orders', v.orders)}${statCard('GMV', fmt(v.gmv))}${statCard('CTR', v.ctr ? v.ctr + '%' : '-')}
      </div>
      ${card('Hiệu suất nội dung', `
        <div class="space-y-2 text-sm">
          ...
          <div class="flex justify-between"><span class="text-slate-500">Published</span><span>${v.published}</span></div>
  ...
  voucher: (id) => {
    ...
    return detailLayout('voucher', id, v.code, 'vouchers', `
      <div class="grid lg:grid-cols-4 gap-4 mb-6">
        ${statCard('Đã dùng', v.used + '/' + v.limit)}${statCard('Chi phí', fmt(v.cost))}${statCard('GMV', fmt(v.gmv))}${statCard('Guardrail', v.guardrail, '', v.guardrail === 'warning' ? 'amber' : 'green')}
      </div>
      ${card('Hiệu suất voucher', `<p class="text-sm">Mã ${v.code} · ${v.discount ? v.discount + '%' : fmtCurrency(v.maxDiscount)}</p>
        <div class="mt-4 flex gap-2">
          ${v.guardrail === 'warning' ? `<button onclick="runAutomationFlow('FLOW_ADS')" class="px-4 py-2 bg-amber-600 text-white rounded-lg text-sm">Giải quyết guardrail</button>` : ''}
